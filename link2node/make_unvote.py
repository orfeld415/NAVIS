...
DATA_PATH = "UNvote.csv"
DIR = "."

# Load new csv 
data = pd.read_csv(f"{DIR}/{DATA_PATH}")
...
data.to_csv(f"{DIR}/tgbn-unvote_edgelist.csv", index=False)
...
# Save labels
aggregated.to_csv(f"{DIR}/tgbn-unvote_node_labels.csv", index=False)

# print number of different labels
print(aggregated['destination'].nunique())
